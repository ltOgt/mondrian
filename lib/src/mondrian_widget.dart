...
abstract class TreeUpdateDetailsAbst {}

/// Details that can be used to update the current [MondrianTree] used in [MondrianWidget] after
/// a leaf has been moved by the user.
///
/// [MondrianTree] does not itself update the tree, so that the caller has control over what exactly happens.
///
/// See [MondrianTree.moveLeaf] which takes the same parameters as are present in this class. // TODO should take this instead
class TreeUpdateDetailsMove extends TreeUpdateDetailsAbst {
  /// The path to the mondrian leaf (actual leaf or tab group) where the [sourceLeafPath] should be moved to.
  final MondrianTreePath targetLeafPath;

  /// The drop position on the [targetLeafPath]
  final MondrianLeafMoveTargetDropPosition targetDropPosition;

  /// The path to the leaf (or tab in tab leaf) that should be moved to [targetLeafPath].
  final MondrianTreePathWithTabIndexIfAny sourceLeafPath;

  TreeUpdateDetailsMove({
    required this.targetLeafPath,
    required this.targetDropPosition,
    required this.sourceLeafPath,
  });
}

/// Details that can be used to update the current [MondrianTree] used in [MondrianWidget] after
/// a node (leaf or branch) has been resized by the user.
///
/// [MondrianTree] does not itself update the tree, so that the caller has control over what exactly happens.
///
/// See [MondrianTree.resizeLeaf] which takes the same parameters as are present in this class. // TODO should take this instead
class TreeUpdateDetailsResize extends TreeUpdateDetailsAbst {
  /// Path pointing to the parent of the node that should be resized
  final MondrianTreePath pathToParent;

  /// New fraction of the node after the user has resized it
  final double newFraction;

  /// Index of the node inside [pathToParent] before the seperator that was used for the resize.
  final int nodeIndexInParent;

  TreeUpdateDetailsResize({
    required this.pathToParent,
    required this.newFraction,
    required this.nodeIndexInParent,
  });
}

/// Details that can be used to update the current [MondrianTree] used in [MondrianWidget] after
/// the focused tab of a [MondrianTreeTabLeaf] has been changed by the user
///
/// [MondrianTree] does not itself update the tree, so that the caller has control over what exactly happens.
///
/// See [MondrianTree.setActiveTab] which takes the same parameters as are present in this class. // TODO should take this instead
class TreeUpdateDetailsTabFocus extends TreeUpdateDetailsAbst {
  /// Path to the [MondrianTreeTabLeaf] which should have its active tab changed.
  final MondrianTreePath pathToTabLeaf;

  /// The tab index of [pathToTabLeaf] that should be the new active tab.
  final int newActiveIndex;

  TreeUpdateDetailsTabFocus({
    required this.pathToTabLeaf,
